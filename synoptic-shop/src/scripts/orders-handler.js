// src/scripts/orders-handler.js - Fixed with centralized auth manager
...
class OrdersHandler {
  ...
  async setupAuthIntegration() {
    try {
      ...
      console.log('👤 Current user from auth manager:', this.currentUser ? this.currentUser.email : 'none');
      
      // Subscribe to auth state changes
      this.authSubscription = authManager.subscribe((user) => {
        console.log('🔄 Orders page: Auth state changed to:', user ? user.email : 'signed out');
      ...
      this.showError('Authentication system error. Please refresh the page.');
    }
  }

  handleAuthStateChange() {
    console.log('🔍 Handling auth state change. User:', this.currentUser ? this.currentUser.email : 'none');
  ...
  async loadOrders() {
    if (!this.currentUser) {
      console.log('❌ Cannot load orders - no authenticated user');
      this.showSection('auth-required');
      return;
    }

    console.log('📦 Loading orders for user:', this.currentUser.email);
    ...
    try {
      ...
      if (this.currentSearch) {
        console.log('🔍 Searching orders with term:', this.currentSearch);
        result = await searchOrders(this.currentSearch, {
          page: this.currentPage,
          limit: 10
        });
      } else {
        console.log('📋 Fetching orders with filter:', this.currentFilter || 'none');
